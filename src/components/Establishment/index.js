          ...
          <ScrollView style={{ height: 600 }}>
            <View style={{ marginHorizontal: 30 }}>
...

              {
                ...
                <Image 
                  style={styles.photo} 
                  source={{uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${establishment.photos[0].photo_reference}&sensor=false&key=GOOGLE_API_KEY_HERE`}} 
                  alt="Store perfil"
                />
                
                :
                  
                <Image style={styles.photo} source={require('../../images/no_photo-1.jpg')} />
              }

              <Text tyle={styles.title}>{props.place.name}</Text>

              {
                (establishment.opening_hours) ?
                  <View>
                    <Text style={{ color: 'white', fontWeight: 'bold', marginTop: 10 }}>
                      {establishment.opening_hours.open_now ? 'Aberto' : 'Fechado'}
                    </Text>
